...
DSA Used:
  - Queue (CustomQueue) : FIFO delivery pipeline: processes parcels in arrival order
...
# CUSTOM DATA STRUCTURE IMPLEMENTED FROM SCRATCH
class CustomQueue:
    def __init__(self):
        """Initialize an empty list to store queue elements from scratch"""
        self.items = []
    ...
    def popleft(self):
        ...
        if not self.is_empty():
            return self.items.pop(0)  # Removes the element at index 0 (the front)
        return None
    
    def remove(self, item):
        """Remove a specific parcel from the queue if it exists"""
    ...
    # These permit our custom class to seamlessly handle native operators 
    # like len(), membership checks ('in'), and for loops.
    def __len__(self):
        """Allows len(delivery_queue) to work seamlessly"""
        return len(self.items)

    def __contains__(self, item):
        """Allows checking membership: 'if parcel in delivery_queue'"""
        return item in self.items

    def __iter__(self):
        """Allows iterating through the queue in loops: 'for parcel in delivery_queue'"""
...
delivery_queue  = CustomQueue()  # Custom Queue built from scratch!
...
def add_to_queue(tracking_id):
    ...
    delivery_queue.append(parcel)    # ENQUEUE: add to back of queue
